...
unsigned short colorAt(int col, int row, const unsigned short *colBitMap, int width) {
    //return ((unsigned char*)colBitMap)[col + width*row];
...
int freeToMove(int col, int row, int width, int height, int dx, int dy,
    ...
    return ((0 <= col + dx && col + dx <= SCREENWIDTH - width) &&       // remains in bounds horizontally
            (0 <= row + dy && row + dy <= SCREENHEIGHT - height) &&     // remains in bounds vertically
            colorAt(col+dx,         row+dy,             colBitMap, 240) &&    // top left corner
            colorAt(col+dx,         row+height+dy,      colBitMap, 240) &&    // bottom left corner
            colorAt(col+width+dx, row+height+dy,      colBitMap, 240) &&    // bottom right corner
            colorAt(col+width+dx, row+dy,             colBitMap, 240) &&    // top right corner
            colorAt(col+dx,         row+(height/2)+dy,  colBitMap, 240) &&    // middle left side
            colorAt(col+width+dx, row+(height/2)+dy,  colBitMap, 240));     // middle right side
}
