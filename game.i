...
# 8 "game.c" 2
# 1 "insructionsMap.h" 1
# 21 "insructionsMap.h"
extern const unsigned short insructionsMapBitmap[19200];


extern const unsigned short insructionsMapPal[256];
# 9 "game.c" 2
# 1 "level1Map.h" 1
# 21 "level1Map.h"
...
# 1 "level2Map.h" 1
# 21 "level2Map.h"
extern const unsigned short level2MapBitmap[19200];
...
# 1 "level3Map.h" 1
# 21 "level3Map.h"
extern const unsigned short level3MapBitmap[19200];
...
extern const unsigned int deathEffect_sampleRate;
extern const unsigned int deathEffect_length;
extern const signed char deathEffect_data[];
# 14 "game.c" 2
# 1 "jumpEffect.h" 1


extern const unsigned int jumpEffect_sampleRate;
extern const unsigned int jumpEffect_length;
extern const signed char jumpEffect_data[];
# 15 "game.c" 2
...
void drawSprites() {
    DMANow(3, shadowOAM, ((OBJ_ATTR *)(0x7000000)), numSprites/2);
}
...
void updatePlayer() {
    ...
    if (player.playerControls) {
        if (player.isCheating) {
            ...
            if ((!(~(oldButtons) & ((1 << 6))) && (~buttons & ((1 << 6)))) && player.canJump) {
                player.vy -= 900;
                player.canJump = 0;
            }
            if ((~((*(volatile unsigned short *)0x04000130)) & ((1 << 5)))) {
            ...
            if (freeToMove(((player.col) >> 8), ((player.row) >> 8),
                        player.width-1, player.height, 0, -1, curColMap)) {
                if (!freeToMove(((player.col) >> 8), ((player.row) >> 8),
                            player.width, player.height-1, 1, 0, curColMap)) {
                ...
                } else if (!freeToMove(((player.col) >> 8), ((player.row) >> 8),
                            player.width, player.height-1, -1, 0, curColMap)) {
            ...
            if (freeToMove(((player.col) >> 8), ((player.row) >> 8),
                ...
                freeToMove(((player.col) >> 8), ((player.row) >> 8),
                player.width, player.height, -1, -1, curColMap)) {
                if (0 < player.vy) {
        ...
        if (freeToMove(((player.col) >> 8), ((player.row) >> 8),
                    player.width, player.height-1, ((player.vx) >> 8), 0, curColMap)) {
            ...
            if (player.vx < 0) {
                for (int i = 0; i >= player.vx; i--) {
                    if (freeToMove(((player.col) >> 8), ((player.row) >> 8),
                                player.width, player.height-1, ((i) >> 8), 0, curColMap)) {
                        dCol = i;
                    }
                }
            }
        ...
        if (freeToMove(((player.col) >> 8), ((player.row) >> 8),
                    player.width-1, player.height, 0, ((player.vy) >> 8), curColMap)) {
        ...
        if (checkWinDie(((player.col) >> 8), ((player.row) >> 8),
            player.width, player.height, curColMap) == 2 && !player.isCheating) {
        ...
        } else if (checkWinDie(((player.col) >> 8), ((player.row) >> 8),
            player.width, player.height, curColMap) == 3) {
    ...
    shadowOAM[player.OAMIndex].attr0 = ((player.row) >> 8) | (2 << 14);
    shadowOAM[player.OAMIndex].attr1 = ((player.col) >> 8) | (0 << 14);
    if (player.direction == LEFT) {
        shadowOAM[player.OAMIndex].attr1 |= (1 << 12);
    }
    shadowOAM[player.OAMIndex].attr2 = (((player.curFrame % 4) * 2)*32 + (player.aniState));
}
...
void updateRecordPlayer() {
    ...
    shadowOAM[recordPlayer.OAMIndex].attr1 = recordPlayer.col | (1 << 14);
    if (recordPlayer.flip) {
        shadowOAM[recordPlayer.OAMIndex].attr1 |= (1 << 12);
    }
    shadowOAM[recordPlayer.OAMIndex].attr2 = (((recordPlayer.curFrame % 4) * 2)*32 + (recordPlayer.aniState)) |
                                            ((1) << 12);

    if (!recordPlayer.showInteraction || player.level == 0) {
        shadowOAM[recordPlayer.OAMIndex+1].attr0 = (2 << 8);
    } else {
        shadowOAM[recordPlayer.OAMIndex+1].attr0 = recordPlayer.row - 8 | (1 << 14);
        shadowOAM[recordPlayer.OAMIndex+1].attr1 = recordPlayer.col | (0 << 14);
        shadowOAM[recordPlayer.OAMIndex+1].attr2 = (((recordPlayer.curFrame % 4))*32 + (recordPlayer.aniState - 4)) |
                                            ((2) << 12);
    }
...
void updateLivesDisplay() {
    ...
    if (player.level != 0) {
        ...
        shadowOAM[livesDisp.OAMIndex].attr1 = livesDisp.col | (1 << 14);
        shadowOAM[livesDisp.OAMIndex].attr2 = (((player.lives - 1) * 2)*32 + (24)) |
                                                ((2 + player.level) << 12);
    } else {
